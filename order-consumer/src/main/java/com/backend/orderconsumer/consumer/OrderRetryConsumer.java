...
@Slf4j
@Component
public class OrderRetryConsumer implements AcknowledgingMessageListener<String, GenericRecord> {

    @Autowired
    private OrderEventsService orderEventsService;

    @Override
    @KafkaListener(topics = {"${topics.retry}"}, autoStartup = "${retryListener.startup:true}", groupId = "retry-listener-group", containerFactory = "retryListenerContainerFactory")
    public void onMessage(ConsumerRecord<String, GenericRecord> consumerRecord, Acknowledgment acknowledgment) {
        log.info("ConsumerRecord in Retry Consumer key : {} , value {}", consumerRecord.key(), consumerRecord.value());
